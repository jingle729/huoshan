package com.effectsar.labcv.algorithm.task.facecluster;
...
public class FaceClusterHandler extends Handler {
    ...
    public FaceClusterHandler(Context context) {
        ...
        if (ret == BEF_RESULT_INVALID_LICENSE) {
            handler.post(() -> ToastUtils.show(context.getResources().getString(R.string.tab_face_cluster) + context.getResources().getString(R.string.invalid_license_file)));
        } else if (ret != BEF_RESULT_SUC) {
            handler.post(() -> ToastUtils.show("FaceCluster Initialization failed"));
        }
    }
    ...
    private BefFaceFeature getFaceFeature(Bitmap bitmap){
        if (bitmap != null){
            ...
            return mFaceVerify.extractFeature(buffer, EffectsSDKEffectConstants.PixlFormat.RGBA8888, bitmap.getWidth(), bitmap.getHeight(), 4 * bitmap.getWidth(), EffectsSDKEffectConstants.Rotation.CLOCKWISE_ROTATE_0);
        }
        return null;
    }
