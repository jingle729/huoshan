package com.effectsar.labcv.algorithm.task.faceverify;
...
public class FaceVerifyThreadHandler extends Handler {
    ...
    private FaceVerifyResult faceVerify(ByteBuffer resizeInputBuffer, int width, int height) {
        EffectsSDKEffectConstants.Rotation rotation = OrientationSensor.getOrientation();
        ...
        if (rotation == EffectsSDKEffectConstants.Rotation.CLOCKWISE_ROTATE_270) {
            rotation = EffectsSDKEffectConstants.Rotation.CLOCKWISE_ROTATE_0;
        }
        long start = System.currentTimeMillis();
        LogTimerRecord.RECORD("faceVerify");
        BefFaceFeature currentFeature = mFaceVerify.extractFeatureSingle(resizeInputBuffer, EffectsSDKEffectConstants.PixlFormat.RGBA8888, width, height, 4 * width, rotation);
