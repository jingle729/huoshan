...
import com.effectsar.labcv.common.adapter.FragmentVPAdapter;
...
//    }
    ...
    public void removeButtonTakePic () {
        getView().findViewById(R.id.iv_record_board).setVisibility(View.GONE);
    }
    ...
    public Fragment getCurrentFragment(){
        ...
        return mAdapter.getItem(mTabSelectedPosition);
    }
    ...
    public void showTabPage(int duration){
        ...
        if(duration > 0){
            tl.animate().alpha(1).setDuration(duration).setListener(new AnimatorListenerAdapter() {
                @Override
            ...
            vp.animate().alpha(1).setDuration(duration).setListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    super.onAnimationEnd(animation);
                    vp.setVisibility(View.VISIBLE);
    ...
    public void hideTabPage(int duration){
        if(duration > 0){
            tl.animate().alpha(0).setDuration(duration).setListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    super.onAnimationEnd(animation);
            ...
            vp.animate().alpha(0).setDuration(duration).setListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    super.onAnimationEnd(animation);
...
//    }
    ...
    public void setRecordOn(boolean on) {
        ivRecordOn = on;
        if (ivRecord == null) {
            return ;
        }
    ...
    public boolean hasValidData(){
        if (mFragmentList != null &&
                mTitleList != null &&
                mTitleList.size() == mFragmentList.size()
        ) {
            return true;
        }
        return false;
