package com.effectsar.labcv.common.gesture;
...
public class GestureManager implements
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                mListener.onTouchEvent(EffectsSDKEffectConstants.TouchEventCode.BEGAN, info.x, info.y, info.force, info.majorRadius, info.pointerId, pointCount);
                break;
            ...
            case MotionEvent.ACTION_UP:
                info = getPointerInfo(event, 0);
                mListener.onTouchEvent(EffectsSDKEffectConstants.TouchEventCode.ENDED, info.x, info.y, info.force, info.majorRadius, info.pointerId, pointCount);
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                for (int i = 0; i < pointCount; i++) {
                    ...
                    mListener.onTouchEvent(EffectsSDKEffectConstants.TouchEventCode.MOVED, info.x, info.y, info.force, info.majorRadius, info.pointerId, pointCount);
                }
                break;
            case MotionEvent.ACTION_CANCEL:
                info = getPointerInfo(event, 0);
                mListener.onTouchEvent(EffectsSDKEffectConstants.TouchEventCode.CANCELLED, info.x, info.y, info.force, info.majorRadius, info.pointerId, pointCount);
                break;
            default:
                break;
        }
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.TAP, transX(e.getX()), transY(e.getY()), 0, 0, 1.f);
        return true;
    }
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.PAN,
                transX(e2.getX()), transY(e2.getY()),
                PreviewSizeManager.getInstance().viewXFactor(dx)*2, // {zh} 计算结果为归一，素材实际为归二，需要*2 {en} The calculation result is normalized, the material is actually normalized to two, and * 2 is required
                PreviewSizeManager.getInstance().viewYFactor(dy)*2, // {zh} 计算结果为归一，素材实际为归二，需要*2 {en} The calculation result is normalized, the material is actually normalized to two, and * 2 is required
                1.f);
        return false;
    }
    ...
    public void onLongPress(MotionEvent e) {
        ...
        mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.LONG_PRESS, transX(e.getX()), transY(e.getY()), 0, 0, 1.f);
    }
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        float tmpScale = mAccScaleFactor / mPreScaleFactor;
            mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.SCALE, tmpScale, 0, 0, 0, mScaleGesture.getScaleFactor());
    ...
    public boolean onDoubleTap(MotionEvent e) {
        ...
        mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.DOUBLE_CLICK, transX(e.getX()), transY(e.getY()), 0, 0, 3.f);
        return false;
    }
    ...
    public boolean onRotate(RotateGestureDetector detector) {
        LogUtils.d("gesture rotate");
            mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.ROTATE, detector.getRotationDegreesDelta() , 0, 0, 0, 6.f);
        return false;
    }
    ...
    public void onRotateEnd(RotateGestureDetector detector) {
        mListener.onGestureEvent(EffectsSDKEffectConstants.GestureEventCode.ROTATE, detector.getRotationDegreesDelta(), 0, 0, 0, 1.f);
    ...
    public interface OnTouchListener {
        void onTouchEvent(EffectsSDKEffectConstants.TouchEventCode eventCode, float x, float y, float force, float majorRadius, int pointerId, int pointerCount);

        void onGestureEvent(EffectsSDKEffectConstants.GestureEventCode eventCode, float x, float y, float dx, float dy, float factor);
    }
}
