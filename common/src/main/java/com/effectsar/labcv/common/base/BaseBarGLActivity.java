...
import com.effectsar.labcv.common.view.BubbleTipManager;
...
public abstract class BaseBarGLActivity extends BaseGLActivity {
    ...
    protected TextView mTimeCost;
    protected TextView mInputSize;
    ...
    private void initView(){
        mBubbleTipManager = new BubbleTipManager(mContext, findViewById(R.id.root_view));
    ...
    protected void hideBoardFragment(Fragment fragment) {
        if (fragment != null) {
            FragmentManager fm = getSupportFragmentManager();
            if (fm.isDestroyed()) {
                LogUtils.e("FragmentManager is destroyed");
                return;
            }

            // Fix: java.lang.IllegalStateException: Can not perform this action after onSaveInstanceState
            if(fm.isStateSaved()) {
                LogUtils.e("FragmentManager isStateSaved");
                return;
            }
    ...
    //   {zh} 收起功能面板，并且也隐藏掉弹出的面板   {en} Collapse the function panel, and also hide the pop-up panel
    protected void hideBoardAndUnderlingFragment(Fragment fragment, boolean show) {
        if (fragment != null) {
            FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
//            ft.setCustomAnimations(R.anim.board_enter, R.anim.board_exit);
            ft.hide(fragment).commitNow();
        }
        showOrHideBoard(show, false);
    }
