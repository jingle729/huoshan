...
import static com.effectsar.labcv.common.config.ImageSourceConfig.ImageSourceType.TYPE_IMAGE;
import static com.effectsar.labcv.common.config.ImageSourceConfig.ImageSourceType.TYPE_VIDEO;
...
import com.effectsar.labcv.common.utils.FrameRator;
...
public abstract class BaseGLActivity extends FragmentActivity implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener, View.OnClickListener {
    ...
    protected volatile boolean mNeedCapture = false;
    ...
    private void parseSrcConfig() {
        ...
        if ((mImageSourceConfig.getFrameRate().equals("30") || mImageSourceConfig.getFrameRate().equals(""))) {
            LocalParamManager.getInstance().setFrameRate(30);
        } else {
            LocalParamManager.getInstance().setFrameRate(Integer.parseInt(mImageSourceConfig.getFrameRate()));
        }
    }

    private void customBySrcConfig() {
        if (null == mImageSourceConfig) return;
        createVideoEncodeHelper();

        if (mImageSourceConfig.getType() == null) return;
        switch (mImageSourceConfig.getType()) {
            case TYPE_CAMERA:
                ...
                break;
            case TYPE_VIDEO:
                createVideoImageSourceProvider();
    ...
    protected void onResume() {
        ...
        mIsPaused = false;
        if (mVideoEncodeHelper != null) {
            mVideoEncodeHelper.setVideoShouldSave(false);
        } else if (mImageSourceConfig.isRecordable()) {
            createVideoEncodeHelper();
            createVideoImageSourceProvider();
        }
    }
    ...
    public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
        ...
        if (mImageSourceProvider instanceof VideoSourceImpl) {
            ...
            LogUtils.e("come to open camera");
            ...
            if (bitmap != null && !bitmap.isRecycled()) {
                if (this.getClass().getSimpleName().equals("QRScanActivity")) {
                    mImageSourceProvider.open(BitmapUtils.cilpWithImage(bitmap, new Size(9, 16), BitmapUtils.BEImageDirectionMode.BEImageClipCenter), null);
                } else {
     ...
     * @return
     */
    protected Bitmap prepareBitmap(String imagePath, int requestWidth, int requestHeight) {
        ...
        if (!isHalDrawOnScreen()) {
            ...
            if (mVideoEncodeHelper != null) {
                mVideoEncodeHelper.onVideoData(EGL14.eglGetCurrentContext(), output.getTexture(), output.getWidth(),
                        output.getHeight(), 30, mImageSourceProvider.getTimestamp());
            }

            //  {zh} 上屏  {en} Top screen
            drawOnScreen(input, output);
        } else {
            if (mVideoEncodeHelper != null) {
                mVideoEncodeHelper.onVideoData(EGL14.eglGetCurrentContext(), output.getTexture(), output.getWidth(),
                        output.getHeight(), 30, mImageSourceProvider.getTimestamp());
    ...
    protected void handlePic(ProcessOutput output) {
        if (mNeedCapture) {
            mNeedCapture = false;
            Bitmap bitmap = mImageUtil.transferTextureToBitmap(output.texture, EffectsSDKEffectConstants.TextureFormat.Texure2D, output.width, output.height);

            ContentResolver contentResolver = BaseGLActivity.this.getContentResolver();
    ...
    protected void drawOnScreen(ProcessInput input, ProcessOutput output) {
        ...
        mImageUtil.drawFrameOnScreen(output.getTexture(), EffectsSDKEffectConstants.TextureFormat.Texure2D, mSurfaceWidth, mSurfaceHeight, drawTransition.getMatrix());
    }
    ...
    protected ProcessInput transToPortrait() {
        ...
        if (mImageSourceProvider.getOrientation() % 180 == 90 || mImageSourceProvider.getTextureFormat() != EffectsSDKEffectConstants.TextureFormat.Texure2D) {
            ...
            if (mImageSourceProvider instanceof CameraSourceImpl) {
                ...
                int texture = mImageUtil.transferTextureToTexture(mImageSourceProvider.getTexture(), mImageSourceProvider.getTextureFormat(), EffectsSDKEffectConstants.TextureFormat.Texure2D,
                        mImageSourceProvider.getWidth(), mImageSourceProvider.getHeight(), transition);
                processInput.setTexture(texture);
            } else {
                // transition and uvMatrix should not use together, use for video input ( use uvMatrix )
                ImageUtil.Transition transition = new ImageUtil.Transition();
                int texture = mImageUtil.transferTextureToTexture(mImageSourceProvider.getTexture(), mImageSourceProvider.getTextureFormat(), EffectsSDKEffectConstants.TextureFormat.Texure2D,
                        mTextureWidth, mTextureHeight, transition, mImageSourceProvider.getUVMatrix());
                processInput.setTexture(texture);
            }
    ...
    protected void startChoosePic() {
        EOUtils.INSTANCE.getPermission().checkPermissions(this, Scene.ALBUM, () -> {
            ...
            AlbumEntrance.INSTANCE.showAlbumPermissionTips(BaseGLActivity.this);
            return null;
        });
    }
    ...
    protected void processSelectedMedia(String path) {
        Intent intent = this.getIntent();
        intent.setClass(this, this.getClass());
        ImageSourceConfig imageSourceConfig = mImageSourceConfig.clone();

        imageSourceConfig.setMedia(path);
        ...
        intent.putExtra("mFilePath_key", mFilePath);
        intent.putExtra("mImgKey", mImgKey);
    ...
    protected void onDestroy() {
        ...
        if (null != mVideoEncodeHelper) {
            sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://" + mVideoEncodeHelper.getVideoPath())));
        }
    ...
    private void createVideoImageSourceProvider() {
        if (mImageSourceConfig.getType() == TYPE_VIDEO) {
            mImageSourceProvider = new VideoSourceImpl(null, new SimplePlayer.IPlayStateListener() {
                ...
                public void onVideoEnd() {
                    runOnUiThread(() -> {
                        if (null != mVideoEncodeHelper) {
                            mVideoEncodeHelper.setVideoShouldSave(true);
                            String videoPath = mVideoEncodeHelper.getVideoPath();
                            ToastUtils.show(getString(R.string.video_process_ok) + videoPath);
                            LogUtils.d(getString(R.string.video_process_ok) + videoPath);
                            saveVideo(videoPath);
                            sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://" + videoPath)));
                        }
                        finish();
    ...
    private void createVideoEncodeHelper() {
