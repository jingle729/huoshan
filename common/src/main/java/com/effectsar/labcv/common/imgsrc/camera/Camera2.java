...
//                    surface.release();
    ...
    public synchronized boolean captureBurst(int width, int height, List<Integer> evs, ImageReader.OnImageAvailableListener listener) {
        if (mPhotoImageReader != null) {
            mPhotoImageReader.close();
            mPhotoImageReader = null;
        }
        ...
        try  {
            ...
            if (sizes != null) {
                ...
                Size s = getClosestCameraPreviewSize((Size[]) suitableSize.toArray(new Size[suitableSize.size()]), new Size(width ,height));
                width = s.getWidth();
                height = s.getHeight();
                sWidth = width;
                sHeight = height;
            ...
            if (sizes != null) {
                getBestMatchCameraPreviewSize((Size[]) suitableSize.toArray(new Size[suitableSize.size()]), new Size(width ,height));
            }
