...
import com.effectsar.labcv.sports.model.SportResult;
import com.effectsar.labcv.sports.task.CountDownTask;
import com.effectsar.labcv.sports.utils.SensorManager;
import com.effectsar.labcv.sports.widgets.BalanceView;
import com.effectsar.labcv.sports.widgets.SportMaskView;
...
public class SportsActivity extends BaseGLActivity implements CountDownTask.CountDownCallback, SensorManager.OnSensorChangedListener, BalanceView.OnBalanceListener, ProgressBar.OnProgressChangedListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ivSet = findViewById(R.id.iv_threshold_set);
        ...
        pb = findViewById(R.id.pb_sports);
        ...
        mRunnable = new Runnable() {
            @Override
            public void run() {
                findViewById(R.id.ll_sports).setVisibility(View.GONE);
            }
        };
        maxValue = mSportItem.getDefaultMaxValue();
...
//        }, 2000);
    ...
    public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
        ...
        setSportThresholdValue(mSportItem);
    }
    ...
    public ProcessOutput processImpl(ProcessInput input) {
        ...
        if (state == STATE_START_RECOGNIZE) {
            BefActionRecognitionInfo.PoseDetectResult result = mAlgorithm.startPose(buffer,
                    ...
                    EffectsSDKEffectConstants.PixlFormat.RGBA8888, mSportItem.readyPoseType(),
    ...
    public void onClick(View view) {
        ...
        else if (view.getId() == R.id.iv_threshold_set) {
            if (findViewById(R.id.ll_sports).getVisibility() == View.GONE) {
                findViewById(R.id.ll_sports).setVisibility(View.VISIBLE);
    ...
    private void setSportThresholdValue(SportItem mSportItem) {
        if (mSportItem == null) {
            return;
        }
        SharedPreferences sp= getSharedPreferences("SportDefaultValue", MODE_PRIVATE);
        float defaultValue = 0;
        switch (mSportItem.getType()) {
//             {zh} 俯卧撑             {en} Push-ups
            case PUSH_UP:
                defaultValue = sp.getFloat(String.valueOf(mSportItem.getType()),0.35f);
                pb.setProgress((defaultValue-minValue)/(maxValue-minValue));
                mAlgorithm.setThreshold(defaultValue);
                break;
//             {zh} 高抬腿跑             {en} Run high
            case HIGH_RUN:
                defaultValue = sp.getFloat(String.valueOf(mSportItem.getType()),0.5f);
                pb.setProgress((defaultValue-minValue)/(maxValue-minValue));
                mAlgorithm.setThreshold(defaultValue);
                break;
            default:
                defaultValue = sp.getFloat(String.valueOf(mSportItem.getType()),0.4f);
                pb.setProgress((defaultValue-minValue)/(maxValue-minValue));
                mAlgorithm.setThreshold(defaultValue);
                break;
        }
    ...
    public void onProgressEnd(ProgressBar progressBar, float progress, boolean isFormUser) {
        int data = mAlgorithm.setThreshold(progress*(maxValue-minValue)+minValue);
        if (data != 0) {
            LogUtils.e(" set sport error");
        }
        saveSportValue(mSportItem, progress);
        mHandler.postDelayed(mRunnable,3000);
    }

    private void saveSportValue (SportItem mSportItem, float progress) {
        SharedPreferences sp = getSharedPreferences("SportDefaultValue", MODE_PRIVATE);
        sp.edit().putFloat(String.valueOf(mSportItem.getType()), progress*(maxValue-minValue)+minValue).apply();// {zh} apply才会写入到xml配置文件里面 {en} Apply will be written to the xml configuration file
    }
}
