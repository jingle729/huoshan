...
public class StepProgressBar extends View {
    ...
    public static final float DEFAULT_MIN_PROGRESS_VALUE = 0.0F;
    ...
    private float mProgressStep = 0f;
    private float mMinProgressValue = 0.0f;
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        mProgressStep = ta.getFloat(R.styleable.ProgressBar_progressStep, DEFAULT_STEP_SIZE);
        mMinProgressValue = ta.getFloat(R.styleable.ProgressBar_minProgressValue, DEFAULT_MIN_PROGRESS_VALUE);
    ...
    private void drawLine(Canvas canvas) {
        ...
        else
        {
            paint.setColor(mActiveLineColor);
            startX = mLeftPadding;
            endX = mLeftPadding + (mWidth - mLeftPadding - mRightPadding) * moveToViewScale(mProgress);
            canvas.drawLine(startX, startY, endX, endY, paint);
    ...
    private void drawCircle(Canvas canvas) {
        ...
        float cx = mLeftPadding + (mWidth - mLeftPadding - mRightPadding) * moveToViewScale(mProgress);
    ...
    private void drawText(Canvas canvas) {
        if (isShowText) {
            ...
            value = moveToStepScale(mProgress);
    ...
    private void calculateProgress(int x) {
        ...
        if (mListener != null) {
            mListener.onProgressChanged(this, moveToStepScale(mProgress), true);
        }
        postInvalidate();
    }
    ...
    private float clip(float progress) {
        if (progress > 1) {
            progress = 1F;
        } else if (progress < mMinProgressValue) {
            progress = mMinProgressValue;
        }
        return progress;
    }

    private float moveToStepScale(float progress) {
        float total = MAX_PROGRESS - mMinProgressValue;
        float stepCount = total / mProgressStep;
        return Math.round(progress * stepCount) * mProgressStep + mMinProgressValue;
    }

    // source is 0-1.0
    private float moveToViewScale(float progress) {
        float total = MAX_PROGRESS - mMinProgressValue;
        float stepCount = total / mProgressStep;
        float pos = Math.round(progress * stepCount);
    ...
    public interface OnProgressChangedListener {
        void onProgressChanged(StepProgressBar progressBar, float progress, boolean isFormUser);
    }
}
