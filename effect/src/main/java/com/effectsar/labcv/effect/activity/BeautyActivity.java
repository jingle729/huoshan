...
//        if (mFeature.equals(FEATURE_AR_HAIR_DYE)) {
//
//        } else {
//            showBoardFragment();
//        }
...
//            effectFragment.setColorListPosition(EffectFragment.BOARD_FRAGMENT_HEAD_ABOVE).useProgressBar(true);
            ...
            effectFragment.setData(mContext,mEffectDataManager, mFilterDatamanager, getLipstickTabItems(),mEffectConfig.getEffectType(), isFirstLaunch());
        }
//        else if (mFeature != null && mFeature.equals(FEATURE_AR_HAIR_DYE)) {
...
//        }
            ...
            effectFragment.setData(mContext,mEffectDataManager, mFilterDatamanager, getTabItems(),mEffectConfig.getEffectType(), isFirstLaunch());
        }
    ...
    public List<EffectFragment.TabItem> getTabItems() {
        return new ArrayList<EffectFragment.TabItem>(){
            {
                if (mFeature.equals(FEATURE_BEAUTY_LITE)) {
                    add(new EffectFragment.TabItem(TYPE_BEAUTY_SUIT, R.string.tab_suit));
                }
                ...
                SharedPreferences sharedPreferences = BeautyActivity.this.getSharedPreferences("license_info_tag", 0);
                if (sharedPreferences.getBoolean("newFilterShowStatus", false)) {
    ...
    public void updateComposerNodeIntensity(Set<EffectButtonItem> effectButtonItems) {
        ...
        if (mSurfaceView != null) {
            mSurfaceView.queueEvent(new Runnable() {
                @Override
                public void run() {
                    for (EffectButtonItem item : effectButtonItems) {
        ...
        if (mSurfaceView != null) {
            mSurfaceView.queueEvent(new Runnable() {
                ...
                public void run() {
                    ...
                    LogUtils.d("updateComposerNodeIntensity +"+node + "  "+key+" "+value);
...
//            if ( (mFeature != null) && (mFeature.equals(FEATURE_AR_LIPSTICK) || mFeature.equals(FEATURE_AR_HAIR_DYE)) ) {
    ...
    public void onMessageReceived(int i, int i1, int i2, String s) {
        ...
        if(i == EffectManager.MSG_ID_DEVICE_ERROR)
        {
            ToastUtils.show(getString(R.string.yunfu_error_tips));
        }
    }
    ...
    protected void setBeautyDefault() {
        if (mEffectFragment != null) {
            ...
            for (EffectButtonItem it : mSelected) {
                if (defaults.contains(it) || null == it.getNode()) {
                    continue;
                }
