...
public class EffectManager {
    ...
    private static boolean MUTE_BGM = false;
    ...
    public int init(ResourceFinder resourceFinder) {
        setCanUsePipeline();
        ...
        String SDKVerison = mRenderManager.getSDKVersion();
        ...
        if (resourceFinder == null) {
            ...
            ret = mRenderManager.init(mContext, resourceFinder, filePath, USE_PIPELINE & canUsePipeLine, onlineLicenseFlag, renderApi);
        }

        if (!checkResult("mRenderManager.init", ret)) return ret;

        mRenderManager.SetBgmMute(MUTE_BGM);
    ...
    /** {zh}
     * @brief 创建特效句柄，确保在gl线程中执行。
     * 只有在初始化不需要鉴权的情况下才使用以下接口，通常情况下用不到.
     * 使用时需要注意调用顺序initPreCheckLicense->initCheckLicense->initPostCheckLicense
     */
    /** {en}
     * @brief Create effects handle to ensure
     * execution in the gl thread。
     * The following interface should only be used when initialization does not require
     * authentication, and is usually not used
     * When using, it is important to pay attention to the order of
     * calling initPreCheckLicense ->initCheckLicense ->initPostCheckLicense
     */
    public int initPreCheckLicense() {
        currentContext = EGL14.eglGetCurrentContext();
        if (currentContext == EGL14.EGL_NO_CONTEXT) {
            LogUtils.e("EffectManager init is not running in GL thread, please check your GL environment!!!");
        }
        ...
        return mRenderManager.initPreCheckLicense(mContext);
    }

    /** {zh}
     * @brief 特效句柄授权，确保在gl线程中执行
     */
    /** {en}
     * @brief Effects handle authorization to ensure
     * execution in the gl thread
     */
    public int initCheckLicense() {
        String filePath = mLicenseProvider.getLicensePath(EffectsSDKLicenseInterface.LICENSE_FUNCTION_NAME.EFFECT);
        boolean onlineLicenseFlag = mLicenseProvider.getLicenseMode() == EffectLicenseProvider.LICENSE_MODE_ENUM.ONLINE_LICENSE;

        return mRenderManager.initCheckLicense(mContext, filePath, onlineLicenseFlag);
    }

    /** {zh}
     * @brief 初始化特效句柄，确保在gl线程中执行
     */
    /** {en}
     * @brief Initialize the effects handle to ensure
     * execution in the gl thread
     */
    public int initPostCheckLicense() {
        setCanUsePipeline();
        ...
        int ret = 0;
        if (USE_MODEL_FROM_ASSET) {
           ret = mRenderManager.initPostCheckLicense(mContext, mResourceProvider.getAssetModelPath(), mContext.getCacheDir().getAbsolutePath(), USE_PIPELINE & canUsePipeLine, true, renderapi);
        } else {
           ret = mRenderManager.initPostCheckLicense(mContext,  mResourceProvider.getModelPath(), mContext.getCacheDir().getAbsolutePath(), USE_PIPELINE & canUsePipeLine, false, renderapi);
        }
        return ret;
    }
...
//       setUseBuiltinSensor(true);
    ...
    public synchronized boolean appendComposeNodes(String[] nodes){
     ...
     */
    public synchronized boolean removeComposeNodes(String[] nodes){
        String prefix = mResourceProvider.getComposePath();
        String[] path = new String[nodes.length];
        for (int i = 0; i < nodes.length; i++) {
            if (!TextUtils.isEmpty(nodes[i]) && !nodes[i].contains(mContext.getPackageName())) {
    ...
    private boolean needLoadResourceSync(String[] nodes) {
        if (null == nodes || nodes.length == 0) {
            return false;
        }
        for (String element : nodes) {
            if (element != null && element.contains("eyelashes")) {
                return true;
            }
        }
        return false;
     ...
     */
    public synchronized boolean setComposeNodes(String[] nodes, String[] tags){
        Iterator<SavedComposerItem>iterator = mSavedComposerNodes.iterator();
        while (iterator.hasNext()){
        ...
        if ((mEnableSyncLoadResource && mNeedLoadResource) || needLoadResourceSync(nodes)){
            if (EGL14.eglGetCurrentContext() != EGL14.EGL_NO_CONTEXT) {
                mRenderManager.loadResourceWithTimeout(-1);
                mNeedLoadResource = false;
            } else {
     ...
     */
    public synchronized boolean updateComposerNodeIntensity(String node, String key, float intensity){
    ...
    public boolean resetSticker() {
        return mRenderManager.resetSticker();
    }
    ...
    public boolean setRenderCacheTexture(String key, int texture, int width, int height){
        return mRenderManager.setRenderCacheExternalTexture(key, texture, width, height) == BEF_RESULT_SUC;
    }

    public void removeRenderCache(String key) {
        mRenderManager.removeRenderCacheExternalTexture(key);
    }
    /** {zh}
     * @param key      key 纹理名称
     *                 name of texture
    ...
    public synchronized boolean setHairColorByPart(int part, float r,float g, float b, float a) {
        try {
    ...
    private void setCanUsePipeline() {
        if (!hasTestUsePipeLine) {
            ...
            canUsePipeLine = PipelineProcessUtils.isSupportPipeline(renderapi, EGL14.eglGetCurrentContext());
