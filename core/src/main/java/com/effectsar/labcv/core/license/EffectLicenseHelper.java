...
public class EffectLicenseHelper implements EffectLicenseProvider {
    ...
    public static final String LICENSE_URL_OFFICIAL = "https://cv.iccvlog.com/cv_tob/v1/api/sdk/tob_license/getlicense"; // {zh} 正式 {en} Formal
    ...
    private static LICENSE_MODE_ENUM _licenseMode = LICENSE_MODE_ENUM.ONLINE_LICENSE;
    private int _errorCode = 0;
    private String _errorMsg = "";
    private EffectsSDKLicenseWrapper licenseWrapper = null;
    private HttpRequestProvider httpProvider = null;

    private static String KEY = "cv_test_online1";
    private static String SECRET = "e479f002-4018-11eb-a1e0-b8599f494dc4";
    public static EffectLicenseHelper getInstance(Context mContext) {
        ...
        if (instance == null) {
            synchronized (EffectLicenseHelper.class) {
                if (instance == null) {
                    //  {zh} 初始化Librarian, 安全加载effect so，开启请咨询技术支持 {en} initialize Librarian, safely load effect.so, please contact technical support
                    // Librarian.init(mContext, BuildConfig.VERSION_NAME, null);
                    // Librarian.loadLibrary("effect");
    ...
    public static void Online_or_offline_model(Context mContext) {
        ...
        if (sharedPreferences.getString("online_model_key", "").equals("ONLINE_LICENSE")) {
            _licenseMode = LICENSE_MODE_ENUM.ONLINE_LICENSE;
            return;
        }
        if (!TextUtils.isEmpty(Config.LICENSE_NAME)) {
            _licenseMode = LICENSE_MODE_ENUM.OFFLINE_LICENSE;
        }
    }
    ...
    private EffectLicenseHelper(Context mContext) {
        ...
        else if (_licenseMode == LICENSE_MODE_ENUM.OFFLINE_LICENSE)
        {
            ...
            parames.put("licensePath", new File(new File(getResourceFile(), "LicenseBag.bundle"), Config.LICENSE_NAME).getAbsolutePath());
        }
    ...
    public String getLicensePath(EffectsSDKLicenseInterface.LICENSE_FUNCTION_NAME function) {
        ...
        if (licenseFileExists()){ // {zh} 文件存在的话，做一次检测，成功的话就返回 {en} If the file exists, do a test and return if successful
            int ret = licenseWrapper.checkLicense(filePath, function, _licenseMode == LICENSE_MODE_ENUM.ONLINE_LICENSE);
            if (ret == 0){
                return filePath;
            }
        }
    ...
    private boolean checkLicenseResult(EffectsSDKLicenseInterface.LICENSE_FUNCTION_NAME function_name) {
        ...
        int ret = licenseWrapper.checkLicense(filename, function_name, _licenseMode == LICENSE_MODE_ENUM.ONLINE_LICENSE);
        if (ret != 0) {
    ...
    private File getResourceFile() {
        return new File(mContext.getExternalFilesDir("assets"), RESOURCE);
    }
