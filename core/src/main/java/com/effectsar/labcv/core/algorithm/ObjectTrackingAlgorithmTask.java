...
public class ObjectTrackingAlgorithmTask extends AlgorithmTask<ObjectTrackingAlgorithmTask.ObjectTrackingResourceProvider, ObjectTrackingAlgorithmTask.ObjectTrackingRenderInfo>
{
    ...
    private ObjectTrackingRenderInfo mObjectTrackingRenderInfo;
    ...
    public void setConfig(AlgorithmTaskKey key, Object p) {
        ...
        if (key.getKey() == OBJECT_TRACKING_TOUCH_EVENT.getKey()) {
            ...
            if (p instanceof AlgorithmTouchInfo) {
                 if (((AlgorithmTouchInfo) p).getAction() == MotionEvent.ACTION_DOWN) {
                     mBoxNeedInitialize = false;
                     mFirstPoint.x = ((AlgorithmTouchInfo) p).getX();
                     mFirstPoint.y = ((AlgorithmTouchInfo) p).getY();

                 } else if (((AlgorithmTouchInfo) p).getAction() == MotionEvent.ACTION_MOVE) {
                     mLastPoint.x = ((AlgorithmTouchInfo) p).getX();
                     mLastPoint.y = ((AlgorithmTouchInfo) p).getY();
