...
import com.effectsar.labcv.core.R;
...
public class ImageQualityPostProcessManager {
        ...
        if (type == EffectsSDKEffectConstants.ImageQualityPostProcessType.IMAGE_QUALITY_POST_PROCESS_TYPE_VFI) {
            ...
            if (on) {
                if (mVideoFI == null) {
                    ...
                    if (!checkResult("Vfi checkLicense", ret)) return ;
                } else {
                    if (mVideoFI != null) {
                        mVideoFI.destroy();
                        mVideoFI = null;
            ...
            if (on) {
                if (mVideoAS == null) {
                    ...
                    if (!checkResult("VideoStab checkLicense", ret)) return;
                } else {
                    mVideoAS.destroy();
                    mVideoAS = null;
                }
            ...
            if (on) {
                if (mVideoDeflicker == null) {
                    ...
                    if (!checkResult("VideoDeflicker checkLicense", ret)) return;
                } else {
    ...
    public int processTexture(PostProcessData processData) {
        ...
        if (mEnableVideoFI && mVideoFI != null) {
            ...
            if (ret == -70) {
                LogUtils.e("no need to process"+ processData.toString());
            } else {
                LogUtils.e("add frame"+ processData.toString());
            }
            if (ret < 0 || ret == -70) {
                return 0;
            }
